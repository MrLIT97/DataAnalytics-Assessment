...
WITH frequency AS (SELECT owner_id, name, Monthly_avg,
		CASE 
...
FROM (
		SELECT owner_id, name, count(*)/count(distinct transaction_month_year) AS Monthly_avg
		FROM (
				SELECT owner_id, CONCAT(u.first_name, ' ', u.last_name) AS name, CONCAT(MONTH(transaction_date), ',', YEAR(transaction_date)) AS transaction_month_year, YEAR(transaction_date) AS transaction_year
